/**
 * Multi-platform instagram — GET /api/multi/instagram
 */
(function () {
      ...
      <article class="unified-card neon-card" data-id="${escapeHtml(item.id)}" style="animation-delay:${rank * 40}ms">
        ...
        <div class="unified-card-body">
          <div class="unified-card-badges">
            <span class="platform-badge platform-badge--instagram">instagram</span>
  ...
  async function load() {
    const res = await fetch("/api/multi/instagram", { credentials: "same-origin" });
  ...
  async function mount(selector) {
    ...
    const empty = document.getElementById("multi-instagram-empty");
    const errEl = document.getElementById("multi-instagram-error");
    if (!root) return;
    try {
      ...
      if (errEl) {
        errEl.hidden = false;
        errEl.textContent = err.message || "Could not load Instagram.";
      }
    }
  }

  window.CrashoutMultiInstagram = { mount, load };
})();
