/**
 * Multi-platform facebook — GET /api/multi/facebook
 */
(function () {
      ...
      <article class="unified-card neon-card" data-id="${escapeHtml(item.id)}" style="animation-delay:${rank * 40}ms">
        ...
        <div class="unified-card-body">
          <div class="unified-card-badges">
            <span class="platform-badge platform-badge--facebook">facebook</span>
  ...
  async function load() {
    const res = await fetch("/api/multi/facebook", { credentials: "same-origin" });
  ...
  async function mount(selector) {
    ...
    const empty = document.getElementById("multi-facebook-empty");
    const errEl = document.getElementById("multi-facebook-error");
    if (!root) return;
    try {
      ...
      if (errEl) {
        errEl.hidden = false;
        errEl.textContent = err.message || "Could not load Facebook.";
      }
    }
  }

  window.CrashoutMultiFacebook = { mount, load };
})();
